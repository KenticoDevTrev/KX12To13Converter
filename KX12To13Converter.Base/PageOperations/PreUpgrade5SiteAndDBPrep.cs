...
namespace KX12To13Converter.Base.PageOperations
{
    public class PreUpgrade5SiteAndDBPrep : IPreUpgrade5SiteAndDBPrep
    {
        ...
        public void RunPreUpgradeFixes(ref string results)
        {
            ...
            RemoveTemplates(ref results);
            DropStyleSheets(ref results);
            CreateMissingContraints(ref results);
            RemovePMTables(ref results);
            CacheHelper.ClearCache();
            SystemHelper.RestartApplication();
        }
...
drop table PM_ProjectTaskStatus
            ...
            {
                results += "<div class='alert alert-error'>Error on RemovePMTables: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
ALTER DATABASE [" + SqlHelper.EscapeQuotes(databaseName) + "] SET COMPATIBILITY_LEVEL = 120;";
            ...
            {
                results += "<div class='alert alert-error'>Error on SetCompatabilityLevel: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
ALTER TABLE [dbo].[Analytics_Statistics] ADD  CONSTRAINT [DEFAULT_Analytics_Statistics_StatisticsObjectName]  DEFAULT (N'') FOR [StatisticsObjectName]
            ...
            {
                results += "<div class='alert alert-error'>Error on Create Missing Constraints: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
delete from [CMS_CssStylesheet]
            ...
            {
                results += "<div class='alert alert-error'>Error on Drop Style Sheets: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
DELETE FROM [CMS_PageTemplate]
	WHERE [PageTemplateID] IN (SELECT PageTemplateID FROM @oldTemplates)
            ...
            {
                results += "<div class='alert alert-error'>Error on Remove Templates: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
DELETE FROM [CMS_Resource] WHERE [ResourceGUID]  =@elementIdentifier
          ...
          END
            ...
            {
                results += "<div class='alert alert-error'>Error on Clear Resources: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
...
-------------------------------------------------------------------------------
          ...
          END
            ...
            {
                results += "<div class='alert alert-error'>Error on ClearUIElements: " + ex.Message.Replace("\n\r", "<br/>") + "</div>";
            }
        }
